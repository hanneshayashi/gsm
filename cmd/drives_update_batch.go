...
var drivesUpdateBatchCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		err = gsmhelpers.CheckBatchFlags(flags, driveFlags, int64(len(csv[0])))
		if err != nil {
			log.Fatalf("Error with batch flag index: %v\n", err)
		}
		l := len(csv)
		results := make(chan *drive.Drive, l)
		...
		go func() {
			for _, line := range csv {
				m := gsmhelpers.BatchFlagsToMap(flags, driveFlags, line, "update")
				maps <- m
			}
			close(maps)
			wg1.Done()
...
func init() {
	drivesUpdateCmd.AddCommand(drivesUpdateBatchCmd)
	flags := drivesUpdateBatchCmd.Flags()
	gsmhelpers.AddFlagsBatch(driveFlags, flags, "update")
	markFlagsRequired(drivesUpdateBatchCmd, driveFlags, "update")
	gsmhelpers.AddFlags(batchFlags, flags, "batch")
	markFlagsRequired(drivesUpdateBatchCmd, batchFlags, "")
}

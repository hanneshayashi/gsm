...
// tokensGetBatchCmd represents the batch command
var tokensGetBatchCmd = &cobra.Command{
	Use:   "batch",
	...
	Run: func(cmd *cobra.Command, args []string) {
		flags, err := gsmhelpers.ConsolidateFlags(cmd, tokenFlags)
		if err != nil {
		...
		err = gsmhelpers.CheckBatchFlags(flags, tokenFlags, int64(len(csv[0])))
		if err != nil {
			log.Fatalf("Error with batch flag index: %v\n", err)
		}
		l := len(csv)
		results := make(chan *admin.Token, l)
		...
		go func() {
			for _, line := range csv {
				m := gsmhelpers.BatchFlagsToMap(flags, tokenFlags, line, "get")
				maps <- m
			}
			close(maps)
			wg1.Done()
...
func init() {
	gsmhelpers.InitBatchCommand(tokensGetCmd, tokensGetBatchCmd, tokenFlags, tokenFlagsALL, batchFlags)
}

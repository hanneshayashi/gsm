...
// userInvitationsCancelBatchCmd represents the batch command
			...
			for i := 0; i < cap; i++ {
				...
				go func() {
					for m := range maps {
						...
						cancelUserInvitationRequest, err := mapToCancelUserInvitationRequest(m)
						if err != nil {
...
func init() {
	gsmhelpers.InitBatchCommand(userInvitationsCancelCmd, userInvitationsCancelBatchCmd, userInvitationFlags, userInvitationFlagsALL, batchFlags)
}

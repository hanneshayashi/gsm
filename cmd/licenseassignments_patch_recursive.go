...
// licenseAssignmentsPatchRecursiveCmd represents the recursive command
var licenseAssignmentsPatchRecursiveCmd = &cobra.Command{
	...
	Long:  "Implements the API documented at https://developers.google.com/admin-sdk/directory/reference/rest/v1/licenseassignments/patch",
			...
			for i := 0; i < threads; i++ {
				...
				go func() {
					for uk := range userKeysUnique {
						...
						result, err := gsmlicensing.PatchLicenseAssignment(productID, skuID, uk, fields, l)
						if err != nil {
							log.Println(err)
						} else {
							results <- result
...
func init() {
	gsmhelpers.InitRecursiveCommand(licenseAssignmentsPatchCmd, licenseAssignmentsPatchRecursiveCmd, licenseAssignmentFlags, recursiveUserFlags)
}

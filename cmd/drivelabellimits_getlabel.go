...
// driveLabelLimitsGetLabelCmd represents the getLabel command
var driveLabelLimitsGetLabelCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, _ []string) {
		...
		result, err := gsmdrivelabels.GetLabelLimits(flags["name"].GetString(), flags["fields"].GetString())
		if err != nil {
			log.Fatalf("Error getting Drive Label limits: %v", err)
		}
...
func init() {
	gsmhelpers.InitCommand(driveLabelLimitsCmd, driveLabelLimitsGetLabelCmd, driveLabelLimitFlags)
}

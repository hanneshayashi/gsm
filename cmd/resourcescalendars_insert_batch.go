...
var resourcesCalendarsInsertBatchCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		err = gsmhelpers.CheckBatchFlags(flags, resourcesCalendarFlags, int64(len(csv[0])))
		if err != nil {
			log.Fatalf("Error with batch flag index: %v\n", err)
		}
		l := len(csv)
		results := make(chan *admin.CalendarResource, l)
		...
		go func() {
			for _, line := range csv {
				m := gsmhelpers.BatchFlagsToMap(flags, resourcesCalendarFlags, line, "insert")
				maps <- m
			}
			close(maps)
			wg1.Done()
...
func init() {
	resourcesCalendarsInsertCmd.AddCommand(resourcesCalendarsInsertBatchCmd)
	flags := resourcesCalendarsInsertBatchCmd.Flags()
	gsmhelpers.AddFlagsBatch(resourcesCalendarFlags, flags, "insert")
	markFlagsRequired(resourcesCalendarsInsertBatchCmd, resourcesCalendarFlags, "insert")
	gsmhelpers.AddFlags(batchFlags, flags, "batch")
	markFlagsRequired(resourcesCalendarsInsertBatchCmd, batchFlags, "")
}

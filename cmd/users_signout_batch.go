...
// usersSignOutBatchCmd represents the batch command
var usersSignOutBatchCmd = &cobra.Command{
	Use:   "batch",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		go func() {
			for _, line := range csv {
				m := gsmhelpers.BatchFlagsToMap(flags, userFlags, line, "signOut")
				maps <- m
			}
			close(maps)
			wg1.Done()
			...
			go func() {
				for m := range maps {
					...
					operation := func() error {
						result, err := gsmadmin.SignOutUser(m["userKey"].GetString())
						if err != nil {
...
func init() {
	gsmhelpers.InitBatchCommand(usersSignOutCmd, usersSignOutBatchCmd, userFlags, userFlagsALL, batchFlags)
}

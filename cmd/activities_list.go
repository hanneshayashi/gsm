...
For more information about the activity report's parameters, see the activity parameters reference guides.`,
	...
	Run: func(cmd *cobra.Command, _ []string) {
		...
		gsmhelpers.StreamOrCollect(gsmreports.ListActivities(flags["userKey"].GetString(), flags["applicationName"].GetString(), flags["actorIpAddress"].GetString(), flags["customerId"].GetString(), flags["endTime"].GetString(), flags["eventName"].GetString(), flags["filters"].GetString(), flags["groupIdFilter"].GetString(), flags["orgUnitId"].GetString(), flags["startTime"].GetString(), flags["fields"].GetString()), streamOutput, compressOutput)
	},
}

...
var calendarsDeleteBatchCmd = &cobra.Command{
	...
	Long:  "Implements the API documented at https://developers.google.com/workspace/calendar/api/v3/reference/calendars/delete",
			...
			for i := 0; i < cap; i++ {
				...
				go func() {
					for m := range maps {
						...
						result, err := gsmcalendar.DeleteCalendar(calendarID)
						if err != nil {
							log.Println(err)
						}
...
func init() {
	gsmhelpers.InitBatchCommand(calendarsDeleteCmd, calendarsDeleteBatchCmd, calendarFlags, calendarFlagsALL, batchFlags)
}

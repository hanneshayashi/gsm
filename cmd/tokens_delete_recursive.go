...
// tokensDeleteRecursiveCmd represents the recursive command
var tokensDeleteRecursiveCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		clientID := flags["clientId"].GetString()
		go func() {
			for i := 0; i < threads; i++ {
				wg.Add(1)
				go func() {
					for uk := range userKeysUnique {
						result, err := gsmadmin.DeleteToken(uk, clientID)
						if err != nil {
							log.Println(err)
						}
...
func init() {
	gsmhelpers.InitRecursiveCommand(tokensDeleteCmd, tokensDeleteRecursiveCmd, tokenFlags, recursiveUserFlags)
}

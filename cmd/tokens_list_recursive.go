...
// tokensListRecursiveCmd represents the recursive command
var tokensListRecursiveCmd = &cobra.Command{
	...
	Long:  "Implements the API documented at https://developers.google.com/workspace/admin/directory/reference/rest/v1/tokens/list",
			...
			for i := 0; i < threads; i++ {
				...
				go func() {
					for uk := range userKeysUnique {
						result, err := gsmadmin.ListTokens(uk, fields)
						if err != nil {
							log.Println(err)
						}
						results <- resultStruct{UserKey: uk, Clients: result}
					}
					wg.Done()
				}()
			}
...
func init() {
	gsmhelpers.InitRecursiveCommand(tokensListCmd, tokensListRecursiveCmd, tokenFlags, recursiveUserFlags)
}

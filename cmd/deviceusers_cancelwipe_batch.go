...
// deviceUsersCancelWipeBatchCmd represents the batch command
			...
			for i := 0; i < cap; i++ {
				...
				go func() {
					for m := range maps {
						...
						result, err := gsmci.CancelDeviceUserWipe(name, m["fields"].GetString(), cancelWipeRequest)
						if err != nil {
							log.Println(err)
						} else {
...
func init() {
	gsmhelpers.InitBatchCommand(deviceUsersCancelWipeCmd, deviceUsersCancelWipeBatchCmd, deviceUserFlags, deviceUserFlagsALL, batchFlags)
}

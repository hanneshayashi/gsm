			...
			for i := 0; i < batchThreads; i++ {
				...
				go func() {
					for m := range maps {
						...
						errKey := fmt.Sprintf("%s - %s:", m["customer"].GetString(), r.RoleName)
...
func init() {
	gsmhelpers.InitBatchCommand(rolesInsertCmd, rolesInsertBatchCmd, roleFlags, roleFlagsALL, batchFlags)
}

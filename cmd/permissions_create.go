...
var permissionsCreateCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		result, err := gsmdrive.CreatePermission(flags["fileId"].GetString(), flags["emailMessage"].GetString(), flags["fields"].GetString(), flags["useDomainAdminAccess"].GetBool(), flags["sendNotificationEmail"].GetBool(), p)
		if err != nil {
...
func init() {
	...
	gsmhelpers.AddFlags(permissionFlags, permissionsCreateCmd.Flags(), permissionsCreateCmd.Use)
	markFlagsRequired(permissionsCreateCmd, permissionFlags, "")
}

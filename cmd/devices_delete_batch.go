...
// devicesDeleteBatchCmd represents the batch command
			...
			for i := 0; i < cap; i++ {
				...
				go func() {
					for m := range maps {
						...
						result, err := gsmci.DeleteDevice(name, customer)
						if err != nil {
							log.Println(err)
						} else {
...
func init() {
	gsmhelpers.InitBatchCommand(devicesDeleteCmd, devicesDeleteBatchCmd, deviceFlags, deviceFlagsALL, batchFlags)
}

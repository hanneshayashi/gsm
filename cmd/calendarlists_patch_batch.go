	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		go func() {
			for _, line := range csv {
				m := gsmhelpers.BatchFlagsToMap(flags, calendarListFlags, line, "patch")
				maps <- m
			}
			close(maps)
			wg1.Done()
			...
			go func() {
				for m := range maps {
					...
					operation := func() error {
						result, err := gsmcalendar.PatchCalendarListEntry(m["calendarId"].GetString(), m["fields"].GetString(), calendarListEntry, m["colorRgbFormat"].GetBool())
						if err != nil {
							log.Println(err)
...
func init() {
	calendarListsPatchCmd.AddCommand(calendarListsPatchBatchCmd)
	flags := calendarListsPatchBatchCmd.Flags()
	gsmhelpers.AddFlagsBatch(calendarListFlags, flags, "patch")
	markFlagsRequired(calendarListsPatchBatchCmd, calendarListFlags, "patch")
	gsmhelpers.AddFlags(batchFlags, flags, "batch")
	markFlagsRequired(calendarListsPatchBatchCmd, batchFlags, "")
}

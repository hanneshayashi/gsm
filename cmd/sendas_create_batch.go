	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		go func() {
			for _, line := range csv {
				m := gsmhelpers.BatchFlagsToMap(flags, sendAsFlags, line, "create")
				maps <- m
			}
			close(maps)
			wg1.Done()
		...
		for i := 0; i < gsmhelpers.MaxThreads(l); i++ {
			...
			go func() {
				for m := range maps {
					...
					errKey := fmt.Sprintf("%s - %s:", m["userId"].GetString(), s.SendAsEmail)
					operation := func() error {
						result, err := gsmgmail.CreateSendAs(m["userId"].GetString(), flags["fields"].GetString(), s)
						if err != nil {
...
func init() {
	sendAsCreateCmd.AddCommand(sendAsCreateBatchCmd)
	flags := sendAsCreateBatchCmd.Flags()
	gsmhelpers.AddFlagsBatch(sendAsFlags, flags, "create")
	markFlagsRequired(sendAsCreateBatchCmd, sendAsFlags, "create")
	gsmhelpers.AddFlags(batchFlags, flags, "batch")
	markFlagsRequired(sendAsCreateBatchCmd, batchFlags, "")
}

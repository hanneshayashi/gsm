...
// verificationCodesListBatchCmd represents the batch command
var verificationCodesListBatchCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		flags, err := gsmhelpers.ConsolidateFlags(cmd, verificationCodeFlags)
		if err != nil {
		...
		l := len(csv)
		results := make(chan []*admin.VerificationCode, l)
		...
		go func() {
			for _, line := range csv {
				m := gsmhelpers.BatchFlagsToMap(flags, verificationCodeFlags, line, "list")
				maps <- m
			}
			close(maps)
			wg1.Done()
...
func init() {
	gsmhelpers.InitBatchCommand(verificationCodesListCmd, verificationCodesListBatchCmd, verificationCodeFlags, verificationCodeFlagsALL, batchFlags)
}

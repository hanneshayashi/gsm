...
// clientStatesPatchBatchCmd represents the batch command
			...
			for i := 0; i < cap; i++ {
				...
				go func() {
					for m := range maps {
						...
						clientState, err := mapToClientState(m)
						if err != nil {
							log.Printf("Error building client state object: %v\n", err)
						}
						result, err := gsmci.PatchClientState(name, customer, m["updateMask"].GetString(), m["fields"].GetString(), clientState)
						if err != nil {
							log.Println(err)
						} else {
...
func init() {
	gsmhelpers.InitBatchCommand(clientStatesPatchCmd, clientStatesPatchBatchCmd, clientStateFlags, clientStateFlagsALL, batchFlags)
}

...
var roleAssignmentsDeleteBatchCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		err = gsmhelpers.CheckBatchFlags(flags, roleAssignmentFlags, int64(len(csv[0])))
		if err != nil {
		...
		go func() {
			for _, line := range csv {
				m := gsmhelpers.BatchFlagsToMap(flags, roleAssignmentFlags, line, "delete")
				maps <- m
			}
			close(maps)
			wg1.Done()
		...
		for i := 0; i < gsmhelpers.MaxThreads(l); i++ {
			...
			go func() {
				for m := range maps {
					var err error
					errKey := fmt.Sprintf("%s - %s:", m["customer"].GetString(), m["roleAssignmentId"].GetString())
...
func init() {
	roleAssignmentsDeleteCmd.AddCommand(roleAssignmentsDeleteBatchCmd)
	flags := roleAssignmentsDeleteBatchCmd.Flags()
	gsmhelpers.AddFlagsBatch(roleAssignmentFlags, flags, "delete")
	markFlagsRequired(roleAssignmentsDeleteBatchCmd, roleAssignmentFlags, "delete")
	gsmhelpers.AddFlags(batchFlags, flags, "batch")
	markFlagsRequired(roleAssignmentsDeleteBatchCmd, batchFlags, "")
}

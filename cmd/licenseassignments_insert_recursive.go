...
// licenseAssignmentsInsertRecursiveCmd represents the recursive command
var licenseAssignmentsInsertRecursiveCmd = &cobra.Command{
	...
	Long:  "Implements the API documented at https://developers.google.com/admin-sdk/directory/reference/rest/v1/licenseassignments/insert",
			...
			for i := 0; i < threads; i++ {
				...
				go func() {
					for uk := range userKeysUnique {
						...
						l.UserId = uk
						result, err := gsmlicensing.InsertLicenseAssignment(productID, skuID, fields, l)
						if err != nil {
							log.Println(err)
						} else {
							results <- result
...
func init() {
	gsmhelpers.InitRecursiveCommand(licenseAssignmentsInsertCmd, licenseAssignmentsInsertRecursiveCmd, licenseAssignmentFlags, recursiveUserFlags)
}

...
var resourcesFeaturesPatchBatchCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		maps, err := gsmhelpers.GetBatchMaps(cmd, resourcesFeatureFlags, batchThreads)
		if err != nil {
			log.Fatalln(err)
		}
		results := make(chan *admin.Feature, batchThreads)
		final := []*admin.Feature{}
		go func() {
...
func init() {
	gsmhelpers.InitBatchCommand(resourcesFeaturesPatchCmd, resourcesFeaturesPatchBatchCmd, resourcesFeatureFlags, resourcesFeatureFlagsALL, batchFlags)
}

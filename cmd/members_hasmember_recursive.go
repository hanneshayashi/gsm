...
// membersHasMemberRecursiveCmd represents the recursive command
			...
			for i := 0; i < threads; i++ {
				...
				go func() {
					for uk := range userKeysUnique {
						result, err := gsmadmin.HasMember(groupKey, uk)
						if err != nil {
							log.Println(err)
						} else {
...
func init() {
	gsmhelpers.InitRecursiveCommand(membersHasMemberCmd, membersHasMemberRecursiveCmd, memberFlags, recursiveUserFlags)
}

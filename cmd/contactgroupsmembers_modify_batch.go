...
var contactGroupsMembersModifyBatchCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		err = gsmhelpers.CheckBatchFlags(flags, contactGroupMemberFlags, int64(len(csv[0])))
		if err != nil {
			log.Fatalf("Error with batch flag index: %v\n", err)
		}
		l := len(csv)
		results := make(chan *people.ModifyContactGroupMembersResponse, l)
		...
		go func() {
			for _, line := range csv {
				m := gsmhelpers.BatchFlagsToMap(flags, contactGroupMemberFlags, line, "modify")
				maps <- m
			}
			close(maps)
			wg1.Done()
...
func init() {
	contactGroupsMembersModifyCmd.AddCommand(contactGroupsMembersModifyBatchCmd)
	flags := contactGroupsMembersModifyBatchCmd.Flags()
	gsmhelpers.AddFlagsBatch(contactGroupMemberFlags, flags, "modify")
	markFlagsRequired(contactGroupsMembersModifyBatchCmd, contactGroupMemberFlags, "modify")
	gsmhelpers.AddFlags(batchFlags, flags, "batch")
	markFlagsRequired(contactGroupsMembersModifyBatchCmd, batchFlags, "")
}
